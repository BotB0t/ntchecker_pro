...
from validator_notification.apps.notification.providers.assemblers import (
    assemble_notification_list,
    assemble_notification_csv,
    assemble_notification_list_to_csv
)
...
class NotificationsProvider:
    ...
    def get_by_user(self, user: User):
        ...
        return sorted(assemble_notification_list(notifications, True), key=lambda x: x['created_at'], reverse=True)
    ...
    def get(self, date_from: datetime = None, date_to: datetime = None, general_id: int = None):
        ...
        return sorted(assemble_notification_list(notifications, get_users_info), key=lambda x: x['created_at'], reverse=True)

    def get_csv(self, response, date_from: datetime = None, date_to: datetime = None, general_id: int = None):
        if general_id:
            notifications = list(IndividualNotification.objects.filter(
                general=general_id))
